...
export async function composeOpsOverview(
  ...
  const groups: OpsGroup[] = selected.map((g) => {
    ...
    const groupMonitors = g.kumaMonitors
      .map((id) => monitorById.get(String(id)))
      .filter((m): m is OpsMonitor => m !== undefined)
    const seen = new Set<string>()
    const systems: OpsGroupSystem[] = []
    for (const ref of g.beszelSystems) {
      const s = systemByRef.get(ref)
      if (s && !seen.has(s.id)) {
        seen.add(s.id)
        systems.push(toGroupSystem(s))
      }
    }
    return { project: g.vercelProject, deployment, monitors: groupMonitors, systems }
  })
